...
public class Parser {
	 ...
	 *
	 */
	private void checkLoop(String data, Stack<String> included)
			throws IOException, ParserMultiException {
		int i = 0;
		do {
	...
	private String preprocessor() throws IOException, ParserMultiException {
		...
		do {
			...
			if (i != -1) {
				...
				if (!(new File(filename)).isAbsolute())
					filename = path + filename;
				filetmp = filetmp.substring(0, i)
						+ fileToString(new BufferedReader(
								new InputStreamReader(new FileInputStream(filename), "ISO-8859-1")))
						+ filetmp.substring(end);
			}
	...
	public void parse(String filename)
			throws FileNotFoundException, SecurityException, IOException, ParserMultiException
	...
	private void doParsing() throws IOException, ParserMultiException {
				...
				try {
					if (line.charAt(i) == '.') {
						...
						if (instr.compareToIgnoreCase(".DATA") == 0) {
							status = 1;
						} else if (instr.compareToIgnoreCase(".TEXT") == 0
								|| instr.compareToIgnoreCase(".CODE") == 0) {
							status = 2;
						} else {
							...
							try {
								if (!((instr.compareToIgnoreCase(".ASCII") == 0)
										|| instr.compareToIgnoreCase(".ASCIIZ") == 0)) {
							...
							if (instr.compareToIgnoreCase(".ASCII") == 0
								...
								try {
									...
									for (String current_string : pList) {
										...
										for (int tmpi = 0; tmpi < num; tmpi++) {
											...
											int to_write = (int) c;
											logger.info("Char: " + c + " (" + to_write + ") ["
													+ Integer.toHexString(to_write) + "]");
											if (escape) {
												switch (c) {
													case '0':
														to_write = 0;
								...
								posInWord++;
									...
									|| instr.compareToIgnoreCase(".WORD64") == 0) {
						...
						if (status != 2) {
							...
							for (int timmy = 0; timmy < Array.getLength(deprecateInstruction); timmy++) {
								if (deprecateInstruction[timmy].toUpperCase()
										.equals(line.substring(i, end).toUpperCase())) {
									...
									if (syntax.charAt(z) == '%') {
										...
										if (syntax.charAt(z) == 'R') // register
										{
											...
											if ((reg = isRegister(param.substring(indPar, endPar).trim())) >= 0) {
												...
												numError++;
												error.add("INVALIDREGISTER", row,
														line.indexOf(param.substring(indPar, endPar)) + 1, line);
												tmpInst.getParams().add(0);
												i = line.length();
												continue;
											}
											...
											if (isImmediate(param.substring(indPar, endPar))) {
												...
												if (isNumber(param.substring(indPar, endPar))) {
													try {
														...
														imm = 0;
														numError++;
														error.add("IMMEDIATE_TOO_LARGE", row,
																line.indexOf(param.substring(indPar, endPar)) + 1, line);
													}
													...
													try {
														try {
															imm = (int) Long
																	.parseLong(Converter.hexToShort(param.substring(indPar, endPar)));
															logger.info("imm = " + imm);
															if (imm < -32768 || imm > 32767)
																throw new NumberFormatException();
														} catch (NumberFormatException ex) {
												...
												try {
													...
													if (cc != -1) {
														...
														if (isNumber(param.substring(cc + 1, endPar))) {
															...
															try {
																try {
																	imm = (int) Long.parseLong(
																			Converter.hexToLong(param.substring(indPar, endPar)));
																	if (imm < -32768 || imm > 32767)
																		throw new NumberFormatException();
																} catch (NumberFormatException ex) {
																	imm = 0;
															...
															MemoryElement tmpMem1 =
																	symTab.getCell(param.substring(cc + 1, endPar).trim());
													...
													numError++;
													error.add("INVALIDIMMEDIATE", row,
															line.indexOf(param.substring(indPar, endPar)) + 1, line);
													i = line.length();
													tmpInst.getParams().add(0);
													continue;
											...
											if (isImmediate(param.substring(indPar, endPar))) {
												...
												if (isNumber(param.substring(indPar, endPar))) {
													try {
														...
														if (imm < 0) {
															numError++;
															error.add("VALUEISNOTUNSIGNED", row,
																	line.indexOf(param.substring(indPar, endPar)) + 1, line);
															i = line.length();
															tmpInst.getParams().add(0);
															continue;
														...
														imm = 0;
														numError++;
														error.add("5BIT_IMMEDIATE_TOO_LARGE", row,
																line.indexOf(param.substring(indPar, endPar)) + 1, line);
													}
													tmpInst.getParams().add(imm);
													indPar = endPar + 1;
													...
													try {
														imm = (int) Long
																.parseLong(Converter.hexToLong(param.substring(indPar, endPar)));
														if (imm < 0) {
															numError++;
															error.add("VALUEISNOTUNSIGNED", row,
																	line.indexOf(param.substring(indPar, endPar)) + 1, line);
											...
											try {
												...
												if (param.substring(indPar, endPar).equals(""))
													...
													if (tmp < 0 || tmp > edumips64.core.CPU.DATALIMIT) {
														...
														error.add(er, row, line.indexOf(param.substring(indPar, endPar)) + 1,
																line);
														i = line.length();
														indPar = endPar + 1;
														tmpInst.getParams().add(0);
										...
										} else if (syntax.charAt(z) == 'E') // Instruction Label
										{
											...
											Integer labelAddr =
													symTab.getInstructionAddress(param.substring(indPar, endPar).trim());
											if (labelAddr != null) {
		...
		for (int i = 0; i < voidJump.size(); i++) {
			...
			if (labelAddr != null) {
				...
				numError++;
				error.add("LABELNOTFOUND", voidJump.get(i).row, voidJump.get(i).column,
						voidJump.get(i).line);
				continue;
			}
		}
	...
	private int isRegister(String reg) {
		...
		try {
			int num;
			if (reg.charAt(0) == 'r' || reg.charAt(0) == 'R' || reg.charAt(0) == '$')// ci sono altri modi
																																								// di scrivere un
																																								// registro???
	 ...
	 * 
	 * @param reg the string to validate
	 * @return -1 if reg isn't a valid alias-register, else a number of register
	 */
	 ...
	 * @param instr params
	 */
	private void writeDoubleInMemory(int row, int i, int end, String line, String instr)
		...
		for (int j = 0; j < Array.getLength(value); j++) {
			...
			/*
			 * if(isHexNumber(value[j])) { try { //insert here support for exadecimal }
			 * catch(NumberFormatException ex) { numError++; error.add("DOUBLE_TOO_LARGE",row,i+1,line);
			 * continue; } catch( Exception e)//modificare in un altro modo { e.printStackTrace(); }
			 * 
			 * } else
			 */ if (b) {
				try {
	 ...
	 */
	private void writeIntegerInMemory(int row, int i, int end, String line, String instr, int numBit,
			String name) throws MemoryElementNotFoundException {
		Memory mem = Memory.getInstance();
		int posInWord = 0; // position of byte to write into a doubleword
		String value[] = instr.split(",");
		MemoryElement tmpMem = null;
		...
		for (int j = 0; j < Array.getLength(value); j++) {
			...
			if (isNumber(value[j])) {
				try {
					...
					if ((num < -(Converter.powLong(2, numBit - 1))
							|| num > (Converter.powLong(2, numBit) - 1)) && numBit != 64)
						throw new NumberFormatException();
				} catch (NumberFormatException ex) {
					numError++;
	...
	private List<String> splitStringParameters(String params, boolean auto_terminate)
