 ...
 * CPU.java
 *
 * This class models a MIPS CPU with 32 64-bit General Purpose Register. (c) 2006 Andrea Spadaccini,
 * Simona Ullo, Antonella Scandura
 *
...
public class CPU {
	...
	/**
	 * CPU status. READY - the CPU has been initialized but the symbol table hasn't been already
	 * filled by the Parser. This means that you can't call the step() method, or you'll get a
	 * StoppedCPUException.
	 * 
	 * RUNNING - the CPU is executing a program, you can call the step() method, and the CPU will
	 * fetch additional instructions from the symbol table
	 *
	 * STOPPING - the HALT instruction has entered in the pipeline. This means that no additional
	 * instructions must be fetched but the instructions that are already in the pipeline must be
	 * executed. THe step() method can be called, but won't fetch any other instruction
	 *
	 * HALTED - the HALT instruction has passed the WB state, and the step() method can't be executed.
	 */
	public enum CPUStatus {
		READY, RUNNING, STOPPING, HALTED
	};
	...
	public void step() throws IntegerOverflowException, AddressErrorException, HaltException,
		 ...
		 * and at the end of this flow the BreakException is re-thrown.
		 */
		int breaking = 0;
	...
	/**
	 * This method resets the CPU components (GPRs, memory,statistics, PC, pipeline and Symbol table).
	 * It resets also the Dinero Tracefile object associated to the current CPU.
	 */
	public void reset() {
	...
	/**
	 * Test method that returns a string containing the values of every register.
	 * 
	 * @returns a string
	 */
