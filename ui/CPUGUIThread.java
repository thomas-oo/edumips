 ...
 * CPUGUIThread.java
 *
 * This class handles the multi-threaded CPU object, and acts as a proxy between the Main class and
 * the CPU class. (c) 2006 Antonella Scandura, Andrea Spadaccini
 *
...
public class CPUGUIThread extends Thread {
	...
	/**
	 * Used to refresh the internal configuration values. Takes the needed configuration values from
	...
	/**
	 * Sets the number of cpu cycles. Set a negative number if you want the CPU to cycle endlessly.
	 * 
	...
	/**
	 * Run method: waits for an external thread that sends the notify. When the notify arrives, the
	 * method will execute nStep CPU cycles if nStep is positive or equal to zero, or it will cycle
	 * indefinitely if nStep is negative.
	 */
	public void run() {
		try {
			while (true) {
				...
				if (nStep < 0) {
					while (true) {
						...
						try {
							...
							JOptionPane.showMessageDialog(f, CurrentLocale.getString(ex.getCode() + ".Message"),
									"EduMIPS64 - " + CurrentLocale.getString("EXCEPTION"), JOptionPane.ERROR_MESSAGE);
							if (terminate) {
								haltCPU();
								break;
							}
							...
							haltCPU();
							JOptionPane.showMessageDialog(edumips64.Main.ioFrame,
									CurrentLocale.getString("ERROR_LABEL"),
									"EduMIPS64 - " + CurrentLocale.getString("ERROR"), JOptionPane.ERROR_MESSAGE);
							break;
						} catch (Exception ex) {
